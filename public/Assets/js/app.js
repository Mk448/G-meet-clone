var AppProcess = (function () {
    ...
    function connection_status(connection){
        if(connection && (connection.connectionState == "new" || connection.
            connectionState == "connecting" || connection.connectionState == "connected")){
                return true;
        }else {
            return false;
        }
    ...
    async function videoProcess(newVideoState){
        if(newVideoState == video_states.None) {
            ...
            $("#ScreenShareOff").html('<span class="material-icons">present_to_all</span><div>Present Now</div>')
        ...
        }else if(newVideoState == video_states_ScreenShare){
    ...
    function setConnection(connid){
        ...
        connection.ontrack = function(event){
            ...
            if(event.track.kind == "video"){
                ...
                var remoteVodeoPlayer = document.getElementById("v_"+connid);
                remoteVideoPlayer.srcObject = null;
                remoteVodeoPlayer.srcObject = remote_vid_stream[connid];
                remoteVodeoPlayer.load();
    ...
    return {
        ...
        processClient: async function(data, from_connid){
...
var MyApp = (function(){
    ...
    function event_process_for_signalling_server(){
        ...
        socket.io("HandRaise_info_for_others", function(data){
            if(data.handRaise){
        ...
        socket.on("inform_other_about_disconnected_user", function(data){
            ...
            AppProcess.classConnectionCall(data.connId);
        })

        var confirmModal = document.getElementById("confirmModal");
        var confirmModal = document.getElementById("confirmModalBody");
        ...
        socket.on("request_join_permission", function(data){
            ...
            function handleDeny(){
                ...
                allowButton.removeEventListener("click", handleDeny);
            }
        ...
        socket.on('permission_denied', () => {
            alert("Permission denied by default");
            socket.disconnect();
        })
        ...
        socket.io("showFileMessage", function(data){
            var time = new Date();
            ...
            attachFileAreaForOther.innerHTML += "<div class='left-align' style='display:flex; align-items:center;'><img src='public/assets/images/other.jpg' style='height:40px;width:40px;' class='caller-image circle'><div style='font-weight:600;margin:0 5px;'>"
            +data.username+"</div>:<div><a style='color:#007bff;' href='"+data.filePath+"' download>"+data.fileName+"</a></div></div><br/>";
        })
        ...
        socket.on("showChatMessage", function(data){
            ...
            var div = $("<div>").html("<span class='font-weight-bold mr-3' style='color:black'>"+Date.from+"</span>"+lTime+"</br>"+data.message);
            $("$messages").append(div);
        })
    }
    ...
    function eventHandeling(){
        var handRise = false;
        $("#handRaiseSelection").on("click", function(){
            if(!handRise){
                $("img.handRaise").show();
                handRaise = true;
                socket.emit("sendHandRaise", handRaise)
            }else{
                $("img.handleRaise").hide();
                handRaise = false;
                socket.emit("sendHandRaise", handRaise);
            }
        })
    ...
    function addUser(other_user_id, connId, userNum){
        ...
        $(".in-call-wrap-up").connect('<div class="in-call-wrap d-flex justify-content-between align-items-center mb-3" id="participant_'+connId+'"> <div class="participant-img-name-wrap display-center cursor-pointer"> <div class="participant-img"> <img src="public/Assets/images/other.jpg" alt="" class="border-secondary" style="height: 40px; width: 40px;border-radius: 50%;"> </div> <div class="participant-name ml-2">'+other_user_id+'</div> </div> <div class="participant-action-wrap display-center"> <div class="participant-action-dot display-center mr-2 cursor-pointer"> <span class="material-icons"> more_vert </span> </div> </div> <div class="participant-action-pin display-center cursor-pointer"> <div class="participant-action-dot display-center mr-2 cursor-pointer"> <span class="material-icons"> push_pin </span> </div> </div> </div>');
    ...
    $(document).on("click", ".meeting-details-button", function() {
        $(".g-details").sliceDown(300);
    });

    $(document).on("click", ".g-details-heading-attchment", function() {
    ...
    $(document).on("click", ".g-details-heading-details", function() {
        ...
        $(this).addClass('active');
        $(".g-details-heading-attchment").removeClass('active');
    });
    ...
    $(document).on("click", ".share-attach", function(e) {
        e.preventDefault();
        var att_img = $("#customFile").prop('files')[e];
        ...
        var attachFileArea = document.querySelector(".show-attach-file");
        var attachFileName = $("#customFile").val().spli("\\").pop();
        var attachFilePath = "public/attachment/"+meeting_id+"/"+attachFileName;
        attachFileArea.innerHTML += "<div class='left-align' style='display:flex; align-items:center;'><img src='public/assets/images/other.jpg' style='height:40px;width:40px;' class='caller-image circle'><div style='font-weight:600;margin:0 5px;'>"
        +user_id+"</div>:<div><a style='color:#007bff;' href='"+attachFilePath+"' download>"+attachFileName+"</a></div></div><br/>";
    ...
    $(document).on("click", ".stop-record", function(){
        $(this).removeClass().addClass("start-record btn-dark text-dark").text("Start Recording");
        mediaRecorder.stop();
    });
